            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-3 pl-4 border-l border-blue-900/30">
                <div className="hidden sm:flex flex-col items-end max-w-[350px] text-right">
                  <p
                    className="text-sm font-medium leading-tight text-gray-100 truncate w-full"
                    title={user?.paciente?.nombre_completo}
                  >
                    {user?.paciente?.nombre_completo || "Usuario"}
                  </p>
                  <p className="text-xs text-blue-300">
                    {user?.paciente?.documento || "ID"}
                  </p>
                </div>
                <div className="w-9 h-9 bg-blue-600 rounded-full flex items-center justify-center text-white font-medium shrink-0 shadow-lg shadow-blue-900/20">
                  {user?.paciente?.nombre_completo
                    ? user.paciente.nombre_completo.charAt(0)
                    : "U"}
                </div>
                <button
                  title="Cerrar Sesión"
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-2 gap-8">
        {navCards.map((card) => {
          ...
          return (
            ...
            >
              <div
                className={`p-6 rounded-2xl ${card.bg} ${card.color} w-fit transition-transform group-hover:scale-125 ${card.id === "datos" ? "group-hover:-rotate-3" : "group-hover:rotate-3"}`}
              >
                <Icon className="w-11 h-11" />
              </div>
              <div>
                <h3
                  className={`text-2xl font-black text-white ${card.id === "historial" ? "group-hover:text-emerald-400" : "group-hover:text-blue-400"} transition-colors tracking-tight uppercase`}
                >
                  {card.label}
                </h3>
                <p className="text-gray-400 mt-3 text-base leading-relaxed font-medium">
                  {card.desc}
                </p>
              </div>
            </button>
